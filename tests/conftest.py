...
def fixture_dir() -> Path:
    """The fixture cache, plus the invalidation rule the doctrine was missing.

    "Fixtures are generated by ffmpeg at test time, never committed" (spec §6)
    reads as a hermeticity guarantee and is one only for the FIRST run on a
    clean tree. Every generator here returns early when its output already
    exists, so a fixture built under one ffmpeg survives the tool being
    upgraded: the suite keeps testing the old tool's output while `cutline
    doctor` reports the new one, and any figure quoted from a fixture (§4.1's
    `0.476009`) silently stops naming the tool that produced it.

    This does not delete anything — a cache the operator did not ask to lose is
    not this fixture's to throw away, and the render outputs under `hfproj/` are
    minutes of work. It REFUSES, naming the directory and the two versions, so
    the staleness is a decision rather than an accident.
    """
    d = Path(__file__).parent / "_fixtures"
    d.mkdir(exist_ok=True)
    stamp = d / "GENERATED-BY.txt"
    current = _ffmpeg_version()
    if stamp.exists():
        recorded = stamp.read_text().strip()
        if recorded != current:
            raise RuntimeError(
                f"the fixtures in {d} were generated by\n    {recorded}\n"
                f"and this run's ffmpeg is\n    {current}\n"
                "A cached fixture built by a different ffmpeg makes every "
                "measurement quoted from it name the wrong tool. Delete the "
                f"directory to regenerate:\n    rm -rf {d}"
            )
    else:
...
@pytest.fixture(scope="session")
def fragmented_source(fixture_dir: Path) -> Path:
    """A crash-safe (fragmented) `.mov` — a container that reports NO
    `nb_frames` on any stream, carrying the same silence pattern as
    `silence_mid` so a cut through it renders for real.

    The seventh fixture class, and it exists because the six before it are all
    normally-muxed MP4: measured, every one of them reports a per-stream frame
    count, so none of them could reach the path where cutline compares a
    directional property against a value it could not read. On this fixture,
    before the fix, `cut()` failed with

        violation: video.nb_frames: None -> 282 (this boundary permits a
                   shrink, not a growth)

    on a completely healthy render — fail-closed working correctly on a
    question it should not have been asked.

    Built by re-muxing (`-c copy`), so the coded frames are byte-identical to
    `silence_mid`'s and the ONLY thing that varies is the container's
    bookkeeping. `tests/test_fixtures.py::test_fragmented_source_reports_no_
    header_frame_count` is the positive control that it really has the defect.
    """
    out = fixture_dir / "fragmented.mov"
    # Same disk-cache hazard as `_build` above: mutating the args below this
    # check has no effect until tests/_fixtures/fragmented.mov is deleted.
    if out.exists():
        return out
    flat = _build(
        fixture_dir / "_frag_flat.mp4",
        volume_filter="volume=enable='between(t,3,5)+between(t,8,9.5)':volume=0",
    )
    _ff(["-i", str(flat), "-c", "copy",
         "-movflags", "+frag_keyframe+empty_moov", str(out)])
    return out


@pytest.fixture(scope="session")
def variable_frame_rate(fixture_dir: Path) -> Path:
    """A genuinely variable-frame-rate source, with silence to cut.

    §2 names phone-shot footage as the ordinary v1 input and the adversarial
    review's F3 observes that the frame-count identity gate assumes a constant
    cadence no fixture could contradict. Measured on a REAL Apple-written
    capture on this machine, that concern is grounded: `r_frame_rate` read
    `15/1` while `avg_frame_rate` read `711600/47429` (15.0035), and another
    read `30/1` against `319080/10637` (29.997).

    Built by dropping frames with `select` + `-fps_mode vfr`, so the presented
    cadence is uneven while the timebase stays 30. `test_fixtures.py::
    test_variable_frame_rate_really_varies` is the positive control.
    """
    out = fixture_dir / "vfr.mp4"
    # Same disk-cache hazard as `_build` above.
    if out.exists():
        return out
    flat = _build(
        fixture_dir / "_vfr_flat.mp4",
        volume_filter="volume=enable='between(t,3,5)+between(t,8,9.5)':volume=0",
    )
    _ff([
        "-i", str(flat),
        "-vf", "select='not(mod(n,3))+gt(mod(n,7),4)'",
        "-fps_mode", "vfr",
