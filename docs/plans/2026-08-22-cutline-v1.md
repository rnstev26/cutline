...
Expected: a 3.12+ version string. If `uv sync` complains about the PEP 639 `license = "Apache-2.0"` string, change that line in `pyproject.toml` to `license = { text = "Apache-2.0" }` and re-run — record which form worked in the commit message.
...
from cutline.verify import CUT_POLICY, COMPOSITE_POLICY, verify
from pathlib import Path
...
def test_v1_keeps_are_speed_one_chunks_only():
    ...
    assert len(edl.keeps) == 2
    assert edl.keeps[0].start == 0 and edl.keeps[0].dur == 97
    assert edl.keeps[1].start == 144 and edl.keeps[1].dur == 103
...
jobs:
  test:
    ...
    steps:
      ...
      - name: Tests
        run: uv run pytest -v --deselect tests/test_flow_cut.py --deselect tests/test_flow_caption.py
```
